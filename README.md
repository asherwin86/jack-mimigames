...
regeneration, that the world survives a restart, and the AI bots (each player picks 0–50; they start 40+ blocks away and only charge once found, shoot arrows from range, hit only their owner at the level chosen, can be killed by anyone, respect the server-wide cap, and leave with their player). To run a PvP server that
